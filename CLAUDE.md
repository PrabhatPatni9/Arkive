...
The full specification is in `ARKIVE_BUILD_BRIEF.md` at the repo root. **Read it in full at
the start of every session and again at the start of each new build phase.** It is the
...
## Workflow
- Maintain `PROGRESS.md` at the root: current phase, what is done, what is next, open
  questions. Update it at the end of every working session.
